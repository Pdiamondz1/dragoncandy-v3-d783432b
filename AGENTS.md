...
* Ignores: `dist/`, `.Codex/**`, `.worktrees/**`, `supabase/**`
...
* **Autoresearch + Donny RAG**: the `/autoresearch` skill (`.Codex/skills/autoresearch/`) grows the wiki and, via `sync-donny`, syncs verified wiki pages into Donny's RAG store (`donny_knowledge`) through the `donny-knowledge-sync` edge function (OpenAI embeddings, idempotent). See `docs/wiki/concepts/self-improving-app.md`.
...
Codex is a **mandatory independent second reviewer** of Codex's code. After Codex's
...
Codex's own reviews — the point is two independent models. (Distinct from
...
Work spanning multiple sessions uses handoff documents in `.Codex/handoffs/`.

**Resuming:** Check `.Codex/handoffs/` for existing handoffs. If user continues prior work, load the freshest relevant handoff. If ambiguous, load and note it. If clearly unrelated, skip.

**Creating:** Invoke `session-handoff` skill when completing a plan phase with more work remaining, switching workstreams, or ending a session with pending work. Skip for small self-contained fixes or fully completed work. After writing the handoff to `.Codex/handoffs/`, also copy it to `docs/wiki/raw/sessions/` and run `/wiki-ops ingest` on the raw session file to synthesize it into the wiki.
...
|-------|---------|----------------|
| Memory (`.Codex/...memory/`) | Durable user/project facts, preferences | When new facts learned |
...
| Handoffs (`.Codex/handoffs/`) | In-flight execution state, next steps | Per work session |
...
Work happens in **git worktrees** under `.Codex/worktrees/` (30+ of them). A worktree is a
...
untracked files aside first. Core files (`AGENTS.md`, `docs/PROJECT_CONTEXT.md`, `docs/wiki/`) appearing
