...
serve(async (req) => {
  ...
  // Donny PROPOSES; a founder approves at /internal/corrections. We capture the
  ...
  if (type === "correction") {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "ingest failed";
      console.error("[aios-report-ingest] correction:", message);
      return json({ error: message }, 500);
    }
  }

  if (type !== "briefing") {
    return json({ error: `Unsupported type "${type}" — this endpoint accepts 'briefing', 'finding', 'findings', or 'correction'` }, 400);
  }
