// wiki-import-doc
// Admin-clicked: server-reads an AIOS Google Workspace Doc and opens a GitHub
// PR creating a NEW docs/wiki/ page from its content, so the document becomes
// durable knowledge Donny recalls after the next donny-knowledge-sync.
//
// - PR-only. Never pushes to base. Never auto-merges.
// - Sibling of wiki-save-answer: saves a DOC (server-read, trusted) rather than
//   a Donny answer (client-provided markdown). Shares the same admin gate, PR
//   ceremony, and 2-folder whitelist.
// - file_exists (page already on base) and github_not_configured are typed 200s.
//
// Bundle must include: _shared/cors.ts, google-workspace.ts, drive-export.ts,
// wiki-import-page.ts

import { serve } from "https://deno.land/std@0.168.0/http/server.ts";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2";
import { corsHeaders } from "../_shared/cors.ts";
import { driveCtx, readDcFile, assertDriveFileId, GoogleWorkspaceError } from "../_shared/google-workspace.ts";
...
// Own, tighter regex — 2 folders only (mirrors wiki-save-answer).
const IMPORT_PATH_RE = /^docs\/wiki\/(concepts|analyses)\/[a-z0-9][a-z0-9-]*\.md$/;
...
function ghHeaders() {
  return {
    ...
    "User-Agent": "dragoncandy-wiki-import",
...
serve(async (req) => {
  ...
  // --- Admin auth (same gate as wiki-save-answer / wiki-commit-pr / aios_corrections_apply) ---
  ...
  if (!IMPORT_PATH_RE.test(path)) return json({ error: "invalid_path" }, 400);
  ...
  try {
    ...
    const content = buildImportedPage({ title, folder, tags, markdown: read.text, fileId, today });
    ...
    // 3. branch — folder+filename-derived ⇒ re-importing the SAME doc recovers
    //    the same branch/PR (idempotent), but `concepts/foo` and `analyses/foo`
    //    map to DISTINCT branches so a same-filename import in another folder
    //    can't fold itself into an unrelated open PR.
    const branch = `donny-wiki-import/${folder}-${filename}`;
    ...
    //    file already there is either THIS import's bytes (idempotent retry) or a
    ...
    const createFile = () =>
      fetch(`${GH}/repos/${REPO}/contents/${path}`, {
        ...
        body: JSON.stringify({
          message: `docs(wiki): import Doc — ${title}`,
          content: toBase64(content),
          branch,
        }),
      });

    // Compare date-agnostically: the only thing that changes for the SAME import
    ...
    // genuinely different import under this filename stays a conflict.
    ...
    const prRes = await fetch(`${GH}/repos/${REPO}/pulls`, {
      ...
      body: JSON.stringify({
        ...
        head: branch,
        base: BASE,
        body: `New knowledge-base page imported from an AIOS Google Workspace doc.\n\nPath: \`${path}\`\nSource doc id: \`${fileId}\`\n\nReview and merge to add it to Donny's RAG on the next sync.`,
      }),
    });
    ...
    return json({ error: `import failed: ${(e as Error).message}` }, 502);
  }
});
