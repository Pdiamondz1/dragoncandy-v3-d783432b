...
export interface SocialDraftCard {
  ...
  data: {
    /**
     * This draft's stable identity, for the once-only guard.
     *
     * The card is persisted verbatim into donny_messages.rich_cards and
     * re-rendered on every conversation load, so "already published" cannot
     * live in component state — a reload would re-arm the button on a draft
     * that is already on a real public feed. The client records this id in
     * donny_draft_publications after a successful publish and reads it back on
     * mount (CT-4b).
     *
     * Generated by the CALLER, not here, so this function stays pure and its
     * tests need no clock or crypto stub — the same injection pattern as
     * buildDonnyProposals' `now`.
     */
    draft_id: string;
