...
describe('buildAwardNotification', () => {
  ...
  it('Oxford-comma joins three or more actions from one run', () => {
    const n = buildAwardNotification([
      { eventType: 'business.first_campaign_created', points: 500 },
      { eventType: 'business.campaign_launched', points: 150 },
      { eventType: 'business.rate_creator', points: 25 },
    ], false);
    expect(n.title).toBe('You earned 675 DC Points');
    expect(n.body).toBe('Created your first campaign, Launched a campaign, and Rated a creator');
  });
