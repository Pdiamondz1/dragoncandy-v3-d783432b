...
// The id is INJECTED, not generated inside buildDraftCard, so this stays a pure
// function with no crypto stub — the same reason buildDonnyProposals takes `now`.
...
describe('buildDraftCard', () => {
  it('names the account by handle and platform', () => {
    const card = buildDraftCard({ draftId: DRAFT_ID, account: IG, caption: 'Taco Tuesday', mediaUrls: [], scheduledAt: null });
    expect(card.type).toBe('social_post_draft');
    expect(card.data.account_label).toBe('@areyouaman · Instagram');
  });

  it('carries the draft id — the key the once-only guard needs (CT-4b)', () => {
    // Without it the card cannot be tied to a donny_draft_publications row, and
    // a reload re-arms the button on a draft already live on a public feed.
    const card = buildDraftCard({ draftId: DRAFT_ID, account: IG, caption: 'x', mediaUrls: [], scheduledAt: null });
    expect(card.data.draft_id).toBe(DRAFT_ID);
    // Never shown to the user, and never handed to the model.
    expect(card.data.account_label).not.toContain(DRAFT_ID);
    expect(draftToolResult(card).text).not.toContain(DRAFT_ID);
  });
  ...
  it('preserves the caption verbatim — what is shown is what posts', () => {
    ...
    const card = buildDraftCard({ draftId: DRAFT_ID, account: IG, caption, mediaUrls: [], scheduledAt: null });
  ...
  it('carries media urls through unchanged', () => {
    ...
    const card = buildDraftCard({ draftId: DRAFT_ID, account: IG, caption: 'x', mediaUrls: urls, scheduledAt: null });
  ...
  it('carries a scheduled time when given one', () => {
    ...
    const card = buildDraftCard({ draftId: DRAFT_ID, account: IG, caption: 'x', mediaUrls: [], scheduledAt: when });
