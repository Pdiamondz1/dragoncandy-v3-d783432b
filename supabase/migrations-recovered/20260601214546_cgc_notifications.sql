...
-- place this migration existed. Version 20260601214546 IS recorded on production.
  ...
  if v_owner is not null then
    begin
      ...
      values (
        ...
        'video',
        jsonb_build_object('promotion_id', NEW.promotion_id, 'submission_id', NEW.id),
        now()
      );
    ...
    if v_owner is not null then
      begin
        ...
        values (
          ...
          '/dashboard/business/promotions/' || NEW.promotion_id::text,
          'checkmark',
          jsonb_build_object('promotion_id', NEW.promotion_id, 'discount_code_id', NEW.id, 'code', NEW.code),
          now()
        );
      exception when others then
        null;
...
-- 5. Add the 'promotions' channel to notification preferences (backfill + default).
