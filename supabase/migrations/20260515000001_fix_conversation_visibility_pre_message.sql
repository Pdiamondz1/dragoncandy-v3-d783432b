-- Fix: Show campaign conversations BEFORE any messages exist.
-- Previously, Branch 2 required at least one message in the campaign before
-- the conversation appeared in the sidebar. This made it impossible for
-- creators/restaurants to initiate messaging during application/counter-offer
-- negotiation because the conversation was invisible.
--
-- Change: the outer EXISTS on messages is now ORed with applicant/owner checks
-- so the conversation appears as soon as an application is submitted.
