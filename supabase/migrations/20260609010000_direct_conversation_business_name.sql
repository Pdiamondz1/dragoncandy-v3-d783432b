-- Fix: direct-conversation participant NAME should be role-appropriate
--
-- The direct branch of get_user_conversations returned profiles.full_name, so a creator
-- DMing a restaurant saw the owner's personal name (e.g. "Joseph Castelo") instead of the
-- business name ("Uncle Rocco") — and, after the avatar fix, the business LOGO sat next to
-- a personal name. The campaign branch already shows the business/creator name, so this
-- brings direct conversations in line.
--
-- Only the direct-branch `other_participant_name` subquery changes vs the previous version
-- (avatar branch already role-aware); everything else is reproduced verbatim.
...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN c.type = 'direct' THEN
      (SELECT COALESCE(
         (SELECT cp_n.creator_name FROM public.creator_profiles cp_n WHERE cp_n.user_id = cp2.user_id LIMIT 1),
         (SELECT bp_n.business_name FROM public.business_profiles bp_n WHERE bp_n.user_id = cp2.user_id LIMIT 1),
