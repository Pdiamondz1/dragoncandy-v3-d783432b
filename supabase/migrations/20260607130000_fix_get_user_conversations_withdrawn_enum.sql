-- 20260607130000_fix_get_user_conversations_withdrawn_enum.sql
-- Fix-forward for a latent bug carried into 20260607120000 (and originating in
-- 20260514000005 / 20260515000001): get_user_conversations filtered
-- `status NOT IN ('rejected', 'withdrawn')`, but 'withdrawn' is NOT a valid
-- application_status enum value (enum = pending/accepted/rejected/counter_offered;
-- no migration ever added 'withdrawn'). Postgres coerces the literal to the enum at
-- execution time, but only when Branch 2 actually evaluates it — i.e. for accounts
-- that HAVE campaigns — throwing
--   invalid input value for enum application_status: "withdrawn"
-- and breaking the messages page for those users. The app deletes applications on
-- withdraw (it never sets that status — see src/hooks/useWithdrawApplication.ts), so
-- no row can ever be 'withdrawn' and NOT IN ('rejected') is exactly equivalent + valid.
-- This recreates the function with the invalid literal removed, preserving the
-- block-exclusion clause added to Branch 1 in 20260607120000.
