...
import { chunkDocument, chunkSourceId, HARD_MAX_CHARS, TARGET_CHARS } from "./chunk-doc.mjs";
...
describe("chunkDocument", () => {
  it("leaves a document that already fits byte-identical", () => {
    const body = "# Small\n\nA short document.";
    expect(chunkDocument("Small", body)).toEqual([`Small\n\n${body}`]);
  });
  ...
  it("keeps every line of a long document, and no chunk exceeds the ceiling", () => {
    ...
    const chunks = chunkDocument("Long Doc", body);
    ...
    const missing = lines(body).filter((l) => !joined.includes(l));
    expect(missing).toEqual([]);
  });

  it("names the document on every chunk, so a chunk retrieved alone still says what it is", () => {
    const body = Array.from({ length: 30 }, (_, i) => `## H${i}\n\n${"x ".repeat(300)}`).join("\n\n");
    const chunks = chunkDocument("Design System", body);
    expect(chunks.length).toBeGreaterThan(1);
  ...
  it("splits on heading boundaries rather than mid-section", () => {
    const body = Array.from({ length: 12 }, (_, i) => `## Rule ${i}\n\n${"y ".repeat(700)}`).join("\n\n");
    const chunks = chunkDocument("Rules", body);
    expect(chunks.length).toBeGreaterThan(1);
    // Each continuation chunk opens (after its title line) on a heading.
  ...
  it("hard-splits a single paragraph bigger than the budget instead of dropping it", () => {
    ...
    const chunks = chunkDocument("Giant", `## One\n\n${giant}`);
    expect(chunks.length).toBeGreaterThan(1);
    for (const c of chunks) expect(c.length).toBeLessThanOrEqual(HARD_MAX_CHARS);
  ...
  it("carries the tail of DESIGN_SYSTEM.md that the old 24k slice dropped", () => {
    const raw = readFileSync(join(REPO, "docs", "DESIGN_SYSTEM.md"), "utf8");
    expect(raw.length).toBeGreaterThan(24_000); // guard: this test is only meaningful oversize

    const chunks = chunkDocument("DESIGN_SYSTEM", raw);
    const joined = chunks.join("\n");
