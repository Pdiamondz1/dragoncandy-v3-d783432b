...
## The load program → [[Synthetic Load Proof (Phase A → 200K)]]

Everything built to *prove scale* on this spine — **Phase A** (the load driver, session pool,
two-lane bulk-seed, economics slice), the **runner matrix Slices 1 and 2** (multi-IP fan-out, real
Range-capped storage egress, the overlap-honest summary RPC), and the live **50K- and 200K-band
runs** — now lives on its own page: [[Synthetic Load Proof (Phase A → 200K)]]. Split out
2026-07-26 when the combined page crossed the RAG embedding ceiling.

The headline it carries: **prod Postgres has never been the constraint** (91% idle at ~312
concurrency, 83% at 1,000, 70% at 4,000 ≈ the 200K-DAU band); every wall hit so far has been
client-side. The teardown rule below (`purge_synthetic_load_cohort()` for load cohorts, NEVER
`purge_synthetic_data()`) is load-bearing for that work.
