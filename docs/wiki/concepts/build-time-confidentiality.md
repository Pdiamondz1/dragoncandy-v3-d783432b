...
created: 2026-08-24
updated: 2026-08-26
sources: [2026-08-24-investor-deck-plan-b.md, 2026-08-26-investor-financial-model-workbook.md]
...
## A filename does not make text confidential (2026-08-26)

The deck gated the EBITDA *number* and printed its **conclusion in English** — "The company's own
line stays negative through 2027" rendered unconditionally. **A value scanner can never catch
prose.** The verifier now says so rather than implying coverage it does not have.

The obvious fix — move the sentence into `trajectory.confidential.tsx` — **would not have worked**,
for the sourcemap reason above. `slides.tsx` imports that file **unconditionally**, so it is in the
public module graph, and with `build.sourcemap` on its entire source ships inside the public `.map`.
Confirmed by finding the full source in `dist/assets/PitchDeck-*.js.map`. **A `*.confidential.tsx`
filename does not make its text confidential, and nothing lints for that** — the `@pitch/confidential`
resolution alias is what makes a module confidential, and only for specifiers it actually intercepts.

## A guard's scope is the thing to check, not its verdict (2026-08-26)

The generated financial workbook shipped the **pre-seed budget**: `Total shared cost`
(686,684 / 775,884 / 775,884) is the budget annualised, and the per-metro allocation and
consolidated EBITDA went with it. Two independent reasons it survived a green
`npm run pitch:verify-public`:

- **`verify-public-bundle.ts` scans `dist/`, the web deck, and nothing else.** The `.xlsx` is
  generated by a Node script, so the vite alias that protects the website does nothing for it. A
  clean report was true and irrelevant.
- **Only the Financing *sheet* had ever been gated, by name.** **Gating one sheet by name does not
  gate the figure it holds** — the same value was reachable from the consolidated rows.

`npm run pitch:verify-public-workbook` (`scripts/verify-public-workbook.ts`) is the sibling check,
two-directional in the same way, and `scripts/lib/public-workbook-guard.ts` holds **one** forbidden
list imported by both the generator (refuses to write) and the verifier (checks what shipped) —
two hand-maintained lists diverge, and the stale one is always the one doing the checking. See
[[Bottom-Up Financial Model]].

## See Also

- [[Investor Pitch Deck & Capital Raise]] · [[Bottom-Up Financial Model]] · [[Site Access Lockdown (Private Preview)]]
