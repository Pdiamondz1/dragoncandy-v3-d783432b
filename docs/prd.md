...
:root {
  ...
  --color-primary-hover: #3BC4AD;
  --color-primary-active: #2AAF99;
  --color-secondary: #F9A8D4;
  --color-secondary-hover: #F78DC2;
  --color-secondary-active: #F472B0;
  --color-bg: #1A1A2E;
  --color-surface: #25253D;
  --color-surface-hover: #2D2D4A;
  --color-neutral: #A8A8A0;
  --color-text: #F5F5F5;
...
**Button — Primary:**
Background: `var(--color-primary)`. Text: white, font-weight 600. Border-radius: 8px. Padding: 12px 24px. Min-height: 44px. Hover: `var(--color-primary-hover)`. Active: `var(--color-primary-active)`. Transition: 150ms ease-out.

**Button — Secondary:**
Background: transparent. Border: 1px solid `var(--color-primary)`. Text: `var(--color-primary)`. Same dimensions as primary.

**Button — Ghost:**
Background: transparent. No border. Text: `var(--color-primary)`. Hover: background `var(--color-surface-hover)`.

**Button — Danger:**
Background: `var(--color-error)`. Text: white. Only for destructive actions.

**Card:**
Background: `var(--color-surface)`. Border-radius: 12px. Padding: 24px. Hover: `var(--color-surface-hover)` background. Transition: 150ms ease-out.

**Input:**
Background: `var(--color-surface)`. Border: 1px solid `rgba(168, 168, 160, 0.3)`. Border-radius: 8px. Padding: 12px. Font-size: 1rem. Focus: border-color `var(--color-primary)`, box-shadow `0 0 0 2px rgba(77, 217, 192, 0.2)`.

**Modal:**
Backdrop: `rgba(0, 0, 0, 0.6)`. Container: `var(--color-surface)`, border-radius 12px, max-width 640px, padding 32px. Entry: fade-in backdrop 150ms, scale-up modal 200ms ease-out from 95%.

**Toast:**
Background: `var(--color-surface)`. Border-left: 4px solid (success/warning/error/info color). Border-radius: 8px. Padding: 16px. Position: bottom-right. Entry: slide-up 200ms ease-out. Auto-dismiss: 5 seconds.

**Badge:**
Border-radius: 9999px. Padding: 4px 12px. Font-size: 0.75rem. Font-weight: 600. Variants: teal (primary), pink (secondary), gray (neutral), green (success), yellow (warning), red (error).
...
export default {
  ...
  theme: {
    extend: {
      colors: {
        'dragon-teal': { DEFAULT: '#4DD9C0', hover: '#3BC4AD', active: '#2AAF99' },
        'candy-pink': { DEFAULT: '#F9A8D4', hover: '#F78DC2', active: '#F472B0' },
        'dragon-bg': '#1A1A2E',
        'dragon-surface': { DEFAULT: '#25253D', hover: '#2D2D4A' },
        'dragon-neutral': '#A8A8A0',
        'dragon-text': { DEFAULT: '#F5F5F5', muted: '#9CA3AF' },
        'dragon-success': '#34D399',
        'dragon-warning': '#FBBF24',
        'dragon-error': '#F87171',
        'dragon-info': '#60A5FA',
      },
      fontFamily: {
