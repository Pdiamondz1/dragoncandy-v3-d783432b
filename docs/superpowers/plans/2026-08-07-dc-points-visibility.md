...
> **Why the shell wrapper:** routes registered as `<ProtectedRoute><Page /></ProtectedRoute>` do **not** get chrome automatically — each page wraps itself, as `NotificationsPage` does. Without `DashboardLayout` the page renders with no nav, no chip, and no padding (`PageBody` deliberately owns only max-width and section rhythm). Verified against `src/pages/NotificationsPage.tsx:60`.
...
> **No double-render.** `DashboardLayout` renders `MobileTopNav` when `isMobile` and its own `<header>` when `!isMobile` (lines 183 and 203) — mutually exclusive. Mounting the chip in both places yields exactly one chip per viewport, which is why both mounts are required rather than redundant.
