...
A new quick action card on `BrandDashboard.tsx`:
- Title: "Social Media"
- Description: "Manage your brand's social presence, amplify sponsored content"
- Style: Teal border, consistent with existing quick action cards
...
2. Join `campaign_applications` WHERE `status` = 'approved' AND `campaign_id` matches
...
| Restaurant name | `profiles` via `campaigns.business_id` |
| Creator name | `profiles` via `campaign_applications.creator_id` |
| Restaurant metrics (posts, reach, impressions, engagement) | `social_posts` + `social_analytics_cache` filtered by restaurant's posts for this campaign |
...
| Cost-per-impression | `campaign_sponsorships.amount` / total impressions |
...
Queries join through `campaign_sponsorships.brand_id` to scope access. No new RLS policies needed if queries run through an Edge Function that validates the brand's sponsorship relationship before returning cross-party data.
...
Existing Browse Creators page. Enhances `CreatorCard` display and adds filter controls.
...
No other schema changes required. All other data reads use existing tables (`campaign_sponsorships`, `social_analytics_cache`, `business_outstand_accounts`, `social_posts`).
...
|-----------|--------------|---------|--------|
| `SponsorshipAmplificationPrompt` | `src/components/social/SponsorshipAmplificationPrompt.tsx` | Modal | Bottom sheet |
| `BrandGuidelinesEditor` | `src/components/social/BrandGuidelinesEditor.tsx` | Inline card | Stacked form |
| `CrossPartyAnalytics` | `src/components/social/CrossPartyAnalytics.tsx` | Two-column | Card list |
| `SponsorshipCard` | `src/components/social/SponsorshipCard.tsx` | Detail panel | Expandable card |
| `CreatorMetricFilters` | `src/components/social/CreatorMetricFilters.tsx` | Horizontal pills | Bottom sheet |
| `SponsorshipMarker` | `src/components/social/SponsorshipMarker.tsx` | Dot + tooltip | Dot + detail |
| `SponsorshipROISummary` | `src/components/social/SponsorshipROISummary.tsx` | Inline card | Stacked card |
...
| `OutstandManager.tsx` | Conditionally render "Sponsorships" tab for brand users |
| `AccountsTab.tsx` | Render `BrandGuidelinesEditor` section for brand users |
| `CreatorCard.tsx` | Add compact verified metrics row |
| Browse Creators page | Add `CreatorMetricFilters` above listing |
