...
**Status:** Slices A + B + C **built** (migrations + seed applied to prod; the `/internal`
Overview card ships with the frontend). Founder-gated go-live remains: the 2 edge-fn deploys
+ `/schedule` the `ai-cost-vs-cap` runner.
