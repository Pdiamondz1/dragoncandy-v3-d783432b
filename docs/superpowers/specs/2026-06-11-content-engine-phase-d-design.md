...
- **`useCreatorBriefPerformance`** (`src/hooks/`) — React Query, key `['creator-brief-performance',
  ...
  })`, returns the RPC rows narrowed to `CreatorBriefPerformanceRow[]` (the `brief` column types as
  `Json` in the generated row; the hook narrows it). Errors thrown per conventions. **Strict
  typecheck requires the function exist in `types.ts`** (the generated `Functions` type is an explicit
  whitelist — an unknown RPC name is a compile error). So we **surgically add the one
  `get_creator_brief_performance` entry** to the `Functions` block (Args + Returns), exactly as
  `resolve_dragonshare_orgs` already appears — a one-function add, **not** a full regen. (Note
  `types.ts` is Lovable-autogenerated; a future regen would re-add it from the live DB, so the manual
...
`content-performance-capture` change; full `types.ts` regen (one surgical function entry only); any
table-RLS change.
