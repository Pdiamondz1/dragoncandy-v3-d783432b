...
Known scaling note (from PR 2 review): the stats RPCs run ~12 independent `count(*)`/`sum()` scans per call — fine at today's ~40 users with the 5-minute client cache, but a scaling cliff around 10k users / 100k+ rows. When weight trends show that approaching, switch the RPCs to read from a counters table or materialized snapshot (the `platform_weight` daily capture already records the row counts needed).
