...
function readmeSheet(): SheetSpec {
  return {
    ...
    rows: [
      ...
      [blank],
      // This said "Every input lives on the Assumptions sheet as a named cell ... Nothing else
      // holds a raw number", which was never true — penetration, subscription revenue and
      // take-rate revenue are all plain values. An overclaim in the first paragraph a reader
      // sees is worse than a smaller true claim, because the reader who checks it stops
      // trusting the sheets that ARE live.
      [t('Pricing, tier mix and cost inputs live on the Assumptions sheet as named cells. Change')],
      [t('one and the metro sheets, the Totals rollup and the ratios that depend on it reflow.')],
      [t('Some cells are deliberately plain values, not formulas — per-metro penetration, the')],
      [t('subscription and take-rate splits, and shared cost. Each is a modelling input or comes')],
      [t('from outside the metro sheets, and each says so where it appears.')],
      [blank],
