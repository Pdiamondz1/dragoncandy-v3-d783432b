...
export const NOTES = {
  ...
  trajectory: {
    ...
    notes:
      'This is built bottom-up, and say so: Census counts the restaurants and bars in each town, we state what share of them we expect to sign, and the revenue falls out of our live pricing. Nothing on this slide is a round number someone picked.\n\nThe bars show METRO CONTRIBUTION, not profit. That is revenue less what the metros themselves cost to run. Company payroll, AI and shared infrastructure sit below it and are not in that number — so 2027 reads positive on the slide while the company is still losing money. Say that before anyone asks; if you let them find it, you look like you were hiding it.\n\n2026 is negative and the company line is negative through 2027. Do not soften either — that is what the raise is for.\n\nIf they ask why it differs from the plan they may have seen: the earlier number was top-down, this one is built from venue counts up, and we did not tune one to match the other. It comes in under the old band and we are showing you that rather than the band. Two things to have ready. First, the bar is revenue BOOKED during the year; exit ARR is where the year ends and is the higher number \u2014 the old plan was stated in ARR, so compare it to exit ARR or you are comparing two different things. Second, almost the whole gap is price per customer, not customer count: the old plan assumed $400-500 a month per restaurant because it counted AI credit overages and rush surcharges, and we book neither, because nobody has ever been charged for them. At the old plan\'s own price our customer count gets to $6.8M. Offer the workbook — every cell traces to its source, and they can change our market-share assumption in the sheet and watch it move.',
  },
  compounds: {
