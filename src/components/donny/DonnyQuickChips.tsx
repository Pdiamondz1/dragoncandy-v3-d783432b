    ...
    <div className="flex gap-2 flex-wrap px-3 py-2">
      {chips.map((chip) => (
        <button
          ...
          disabled={disabled}
          className="bg-white border border-[#4DD9C0] text-[#4DD9C0] text-xs font-medium px-3 py-1.5 rounded-full hover:bg-[#4DD9C0] hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed focus-visible:ring-2 focus-visible:ring-[#4DD9C0]"
        >
          {chip.label}
        </button>
      ))}
