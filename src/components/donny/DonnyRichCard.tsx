...
export function DonnyRichCard({ card }: DonnyRichCardProps) {
  ...
  switch (card.type) {
    case 'creator_profile':
      return (
        <div className="bg-white/5 rounded-xl p-3 border-2 border-teal-300 mt-1.5">
          <div className="flex gap-2 items-center">
            <div className="w-10 h-10 rounded-full bg-white/10 ring-2 ring-teal-400 overflow-hidden flex-shrink-0">
            ...
            <div>
              <div className="text-sm font-bold text-white">{card.data.name}</div>
              <div className="text-xs text-white/60">{(card.data.platforms ?? []).join(' · ')}{card.data.niche ? ` · ${card.data.niche}` : ''}</div>
    ...
    case 'campaign_summary':
      return (
        <div className="bg-white/5 rounded-xl p-3 border-2 border-teal-300 mt-1.5">
          <div className="text-sm font-bold text-white">{card.data.title}</div>
    ...
    case 'payment_confirmation':
      return (
        <div className="bg-white/5 rounded-xl p-3 border-2 border-dc-yellow mt-1.5">
          <div className="text-sm font-bold text-white">Payment Ready</div>
          <div className="text-xs text-white/60 mt-1">
            ${card.data.amount} to {card.data.recipient_name}
          </div>
          <div className="text-xs text-white/60">{card.data.description}</div>
          <Button
            size="sm"
            className="w-full rounded-full bg-dc-teal-btn hover:bg-dc-teal-btn-hover text-white text-xs mt-2"
    ...
    case 'application_summary':
      return (
        <div className="bg-white/5 rounded-xl p-3 border-2 border-teal-300 mt-1.5">
          <div className="text-sm font-bold text-white">{card.data.campaign_title}</div>
          <div className="text-xs text-white/60 mt-1">From: {card.data.creator_name}</div>
          <div className="text-xs text-white/60 italic mt-1">"{card.data.pitch}"</div>
