...
export function OnboardingProgress({ currentStep, totalSteps, accentColor }: OnboardingProgressProps) {
  const fill = accentColor === 'teal' ? 'bg-landing-mint' : 'bg-landing-pink';
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
          ...
          return (
            <motion.div
              ...
              className={`h-2 rounded-full transition-colors duration-300 ${
                isActive ? fill : isComplete ? `${fill} opacity-60` : 'bg-landing-line'
              }`}
      ...
      {/*
        ...
        unlabelled divs.
      */}
      <p className="text-[11px] font-medium text-landing-ink-soft" aria-live="polite">
