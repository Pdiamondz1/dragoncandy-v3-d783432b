...
function RewardsTeaser() {
  ...
  return (
    <Reveal delay={0.12}>
      <div className="relative mt-10 overflow-hidden rounded-[2rem] border border-dc-pink-accent/20 bg-white bg-gradient-to-br from-dc-pink-accent/10 via-white to-dc-teal/10 p-6 sm:p-8 lg:p-10">
        ...
        <div className="relative">
          ...
          <h3 className="max-w-xl text-2xl font-extrabold tracking-tight text-dc-text sm:text-3xl">
            Every move earns{" "}
              ...
              return (
                <div key={key} className="flex items-center gap-3">
                  ...
                  {i < TIER_ORDER.length - 1 && (
                    <ChevronRight className="h-4 w-4 shrink-0 text-dc-text-muted/70" aria-hidden />
                  )}
                </div>
              );
            })}
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {earnExamples.map((ex) => (
              <div
                key={ex.label}
                className="flex items-center justify-between gap-3 rounded-2xl border border-dc-text/10 bg-white px-4 py-3"
              >
                <span className="text-sm text-dc-text-muted">{ex.label}</span>
    ...
    <section id="proof" className="py-20 lg:py-32">
      <div className="mx-auto max-w-6xl px-5 sm:px-8 lg:px-12">
        <Reveal>
          ...
          <h2 className="max-w-3xl text-3xl font-extrabold tracking-tight text-dc-text sm:text-4xl lg:text-5xl">
        ...
        <Reveal delay={0.06}>
          <div className="mt-8 flex flex-wrap gap-3">
            {trustChips.map((chip) => (
              <span
                key={chip}
                className="inline-flex items-center gap-2 rounded-full border border-dc-text/10 bg-white px-4 py-2 text-sm font-medium text-dc-text-muted shadow-sm"
              >
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {testimonials.map((story, i) => (
              <Reveal key={story.name} delay={i * 0.08}>
                <figure className="flex h-full flex-col rounded-3xl border border-dc-text/10 bg-white p-6 shadow-sm">
                  <blockquote className="flex-1 text-base leading-relaxed text-dc-text-muted">
                    &ldquo;{story.quote}&rdquo;
                  </blockquote>
                  <figcaption className="mt-6">
                    <p className="text-sm font-bold text-dc-text">{story.name}</p>
                    <p className="text-xs text-dc-text-muted/70">{story.role}</p>
                  </figcaption>
                </figure>
              </Reveal>
            ))}
