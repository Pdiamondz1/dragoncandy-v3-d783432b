...
export interface PlatformStats {
  ...
  // Every `*_all` field is OPTIONAL: they all ship together from the `aios_platform_stats` totals
  // migration, so a pre-migration RPC omits the whole set. Consumers must tolerate their absence
  // (the model's ofTotal/synthValue helpers accept undefined; the banner guards with `?? 0`).
