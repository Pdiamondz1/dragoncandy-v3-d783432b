...
/**
 * The privacy policy's TEXT, with no layout, no hooks and no providers.
 *
 * It lives apart from `PrivacyPolicy.tsx` so `scripts/build-legal-static.ts` can
 * render it to `public/privacy.html` with `renderToStaticMarkup` and no React
 * context at all. That static file is what the site gate allowlists and what the
 * Google / Meta / TikTok / X app reviews are pointed at — each requires an
 * anonymously reachable privacy policy, and `/privacy` is a SPA route that
 * answers 401 the moment `SITE_GATE_ENABLED` is on.
 *
 * **The split exists to prevent a second copy of the policy, not for tidiness.**
 * A hand-written static page would be a fork of a legal document that nothing
 * keeps in step — and the one field guaranteed to change, `PRIVACY_LAST_UPDATED`,
 * is the one a reader uses to decide whether to trust it. One source, two
 * renderers; `src/pages/legal/privacyStatic.test.tsx` fails if the committed HTML
 * falls behind this file.
 *
 * Keep this component hook-free. A `useNavigate`, a `useAuth`, or anything that
 * reads context turns the generator into a provider-stubbing exercise and will
 * break the build script rather than the app, which is the worse place to find out.
 */

/** Bumped whenever the policy text changes; §10 commits to it. Kept in step with the Terms. */
