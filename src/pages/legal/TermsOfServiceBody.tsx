...
/**
 * The terms of service' TEXT, with no layout, no hooks and no providers — the same
 * split, and for the same reason, as [PrivacyPolicyBody]. Read that file's comment
 * first; it carries the full reasoning.
 *
 * **Why terms needs a static copy too, which was not obvious.** The privacy page
 * shipped first because four platform reviews name a privacy policy explicitly. But
 * every console that asks for a privacy URL asks for a **terms** URL on the same
 * form, and a 401 there is an anonymously inaccessible legal URL in a live
 * submission. The first version of the TikTok runbook hedged this as "TikTok does
 * not appear to fetch it" — an assumption about a reviewer's behaviour, which is not
 * a basis for calling a platform unblocked. Caught by the Codex second review.
 *
 * Keep this component hook-free; see [PrivacyPolicyBody] for what breaks otherwise.
 */

/** Bumped whenever the terms change; §16 commits to it. Kept in step with the Privacy Policy. */
