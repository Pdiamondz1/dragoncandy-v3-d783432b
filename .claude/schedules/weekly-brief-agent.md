...
6. **File it**: read the bearer via `execute_sql`: `select decrypted_secret from vault.decrypted_secrets where name = 'aios_ingest_key'`, then POST `https://zocahiffooqdybdhguqv.supabase.co/functions/v1/aios-report-ingest` with body `{"type":"briefing","payload":{week_start, title, body_md, kpis, "generated_by":"weekly-brief-agent"}}`. Omit `publish` — an admin reviews and publishes from /internal/briefings.

7. **Verify**: `select id, week_start, title from aios_briefings where week_start = '<week_start>'` and report the row id. If the POST failed, report the error — do not retry more than twice and do not fall back to direct table writes.
